...
//                .antMatchers("/adminHome").permitAll() // 로그인 권한은 누구나, resources파일도 모든권한
//                .antMatchers("/admin", "/signUp", "/createAccount", "/idCheck", "/login",
//                        "/index", "/memberList", "").hasAnyRole("ADMIN", "MANAGER") // 괄호의 권한을 가진 유저만 접근가능, ROLE_가 붙어서 적용 됨. 즉, 테이블에 ROLE_권한명 으로 저장해야 함.
//                .antMatchers("/managerList").hasRole("ADMIN")
